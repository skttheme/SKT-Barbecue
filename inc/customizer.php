<?php
/**
 * SKT Barbecue Theme Customizer
 *
 * @package SKT Barbecue
 */
 
function skt_barbecue_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'skt_barbecue_custom_header_args', array(
		'default-text-color'     => '949494',
		'width'                  => 1600,
		'height'                 => 230,
		'wp-head-callback'       => 'skt_barbecue_header_style',
 		'default-text-color' => false,
 		'header-text' => false,
	) ) );
}
add_action( 'after_setup_theme', 'skt_barbecue_custom_header_setup' );
if ( ! function_exists( 'skt_barbecue_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog
 *
 * @see skt_barbecue_custom_header_setup().
 */
function skt_barbecue_header_style() {
	?>    
	<style type="text/css">
	<?php
...
endif; // skt_barbecue_header_style 
...
function skt_barbecue_customize_register( $wp_customize ) {
	//Add a class for titles
    class skt_barbecue_Info extends WP_Customize_Control {
        public $type = 'info';
        public $label = '';
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'color_scheme',array(
			'label' => esc_html__('Color Scheme','skt-barbecue'),			
			'section' => 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'header_bg_color',array(
			'label' => esc_html__('Header Background Color','skt-barbecue'),				
			'section' => 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'footer_text_color',array(
			'label' => esc_html__('Copyright Text Color','skt-barbecue'),				
			'section' => 'colors',
	...
	// Header Button
		 $wp_customize->add_section('header_button',array(
			'title'	=> esc_html__('Header Button','skt-barbecue'),					
			'priority'		=> null
	));
	...
	$wp_customize->add_control('header_btntext',array(
			'label'	=> esc_html__('Button Text','skt-barbecue'),
	...
	$wp_customize->add_control('header_btn_link',array(
			'label'	=> esc_html__('Button Link','skt-barbecue'),
			'section'	=> 'header_button',
	...
	$wp_customize->add_setting('hide_header_btn',array(
			'sanitize_callback' => 'skt_barbecue_sanitize_checkbox',
			'default' => true,
	));	 
	$wp_customize->add_control( 'hide_header_btn', array(
    	   'section'   => 'header_button',    	 
		   'label'	=> esc_html__('Uncheck To Show Button In Header','skt-barbecue'),
    	   'type'      => 'checkbox'
     )); 	
	 // Hide Header Button 
	...
	$wp_customize->add_section('header_transparent',array(
			'title'	=> esc_html__('Homepage Header Transparent','skt-barbecue'),					
			'priority'		=> null
	));	
	...
	$wp_customize->add_control( 'option_header_transparent', array(
		   ...
		   'label'	=> esc_html__('Uncheck To Enable Transparent Header.','skt-barbecue'),
		   'type'      => 'checkbox'
	 ));	
	 // Transparent Header	
	...
	$wp_customize->add_section('header_transparent_inner',array(
			'title'	=> esc_html__('Category / Archive, Inner Page & Single Post Header Transparent','skt-barbecue'),					
			'priority'		=> null
	));	
	...
	$wp_customize->add_section('inner_page_banner',array(
			'title'	=> esc_html__('Inner Page Banner Settings','skt-barbecue'),					
			'priority'		=> null
	));	
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'inner_page_banner_thumb', array(
		...
		'label'	=> esc_html__('Upload Default Banner Image','skt-barbecue'),
	...
	$wp_customize->add_control( 'inner_page_banner_option', array(
    	   'section'   => 'inner_page_banner',    	 
		   'label'	=> esc_html__('Uncheck To Show Inner Page Banner On All Inner Pages. For Display Different Banner Image On Each Page Set Page Featured Image. Set Image Size (1400 X 360) For Better Resolution.','skt-barbecue'),
    	   'type'      => 'checkbox'
     ));	
	 // Inner Page Banner Settings
	...
	$wp_customize->add_section('inner_post_banner',array(
			'title'	=> esc_html__('Category / Archive And Single Post Banner Settings','skt-barbecue'),					
			'priority'		=> null
	));	
	...
	$wp_customize->add_control( 'inner_post_banner_option', array(
    	   'section'   => 'inner_post_banner',    	 
		   'label'	=> esc_html__('Uncheck To Show Inner Post Banner On Category / Archive And Single Post. For Display Different Banner Image On Each Post Set Post Featured Image. Set Image Size (1400 X 360) For Better Resolution.','skt-barbecue'),
    	   'type'      => 'checkbox'
     ));	
	 // Inner Page Banner Settings	
	...
	$wp_customize->add_section('footer_bar',array(
			'title'	=> esc_html__('Footer Info Box','skt-barbecue'),					
			'priority'		=> null
	));
    ...
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'footer_logo_image_control', array(
        'label'	=> esc_html__('Footer Logo','skt-barbecue'),
        'section' => 'footer_bar',
	...
	$wp_customize->add_control('footer_logo_url',array(
			'label'	=> esc_html__('Footer Logo Link','skt-barbecue'),
			'section'	=> 'footer_bar',
	...
	$wp_customize->add_control( 'hide_footer_bar', array(
    	   'section'   => 'footer_bar',    	 
		   'label'	=> esc_html__('Uncheck To Show This Section','skt-barbecue'),
    	   'type'      => 'checkbox'
     )); 	
	 // Hide Footer Info Box	 
	...
	$wp_customize->add_section('footer_text_copyright',array(
			'title'	=> esc_html__('Footer Copyright Text','skt-barbecue'),				
			'priority'		=> null
	));
	...
	$wp_customize->add_control('footer_text',array(
			'label'	=> esc_html__('Add Copyright Text Here','skt-barbecue'),
			'section'	=> 'footer_text_copyright',
			'setting'	=> 'footer_text'
	));		 
}
add_action( 'customize_register', 'skt_barbecue_customize_register' );
//Integer
function skt_barbecue_sanitize_integer( $input ) {
    if( is_numeric( $input ) ) {
        return intval( $input );
    }
}
function skt_barbecue_sanitize_checkbox( $checked ) {
	// Boolean check.
	return ( ( isset( $checked ) && true == $checked ) ? true : false );
}

//setting inline css.
function skt_barbecue_custom_css() {
    wp_enqueue_style(
        'skt-barbecue-custom-style',
        get_stylesheet_directory_uri() . '/css/skt-barbecue-custom-style.css'  
    );
        ...
        $custom_css = "
				...
				";
        wp_add_inline_style( 'skt-barbecue-custom-style', $custom_css );
}
add_action( 'wp_enqueue_scripts', 'skt_barbecue_custom_css' );          
/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function skt_barbecue_customize_preview_js() {
	wp_enqueue_script( 'skt_barbecue_customizer', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20130508', true );
}
add_action( 'customize_preview_init', 'skt_barbecue_customize_preview_js' );